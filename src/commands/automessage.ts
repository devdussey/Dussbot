...
const store = requireFromSrcIfNeeded('../utils/autoMessageStore');
const scheduler = requireFromSrcIfNeeded('../utils/autoMessageScheduler');
...
function parseImageUrl(input: string | null) {
  if (!input) return null;
  const trimmed = input.trim();
  if (!trimmed) return null;
  try {
...
const command: SlashCommandModule = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) => opt.setName('message').setDescription('Plain text message to send'))
        .addStringOption((opt) => opt.setName('embed_title').setDescription('Embed title (optional)'))
        .addStringOption((opt) => opt.setName('embed_description').setDescription('Embed description (optional)'))
        .addStringOption((opt) => opt.setName('embed_footer').setDescription('Embed footer text (optional)'))
        .addStringOption((opt) => opt.setName('embed_color').setDescription('Hex colour for the embed (e.g. #5865F2)'))
        .addStringOption((opt) => opt.setName('embed_image').setDescription('Image or GIF URL for the embed')))
    .addSubcommand((sub) =>
      sub
        .setName('delete')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'create') {
      ...
      const intervalMs = Math.max(1, hours) * 3600000;
      ...
      try {
        scheduler.startJob(interaction.client, guildId, job);
      } catch (_) {}
