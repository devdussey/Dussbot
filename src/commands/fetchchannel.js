...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('fetchchannel')
    .setDescription('Fetch channel history into the word search store')
  ...
  async execute(interaction) {
    ...
    summaryLines.push(`Stored ${formatCount(stored, 'message')} across ${formatCount(uniqueUsers, 'user')} in the word search store.`);
    if (skippedBots > 0) {
