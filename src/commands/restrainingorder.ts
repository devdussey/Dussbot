...
const store = requireFromSrcIfNeeded('../utils/restrainingOrderStore');
...
async function resolveUserTag(client: any, userId: string) {
...
const command: SlashCommandModule = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addUserOption((opt) => opt.setName('user1').setDescription('First user').setRequired(false))
        .addStringOption((opt) => opt.setName('user1id').setDescription('First user ID').setRequired(false))
        .addUserOption((opt) => opt.setName('user2').setDescription('Second user').setRequired(false))
        .addStringOption((opt) => opt.setName('user2id').setDescription('Second user ID').setRequired(false)))
    .addSubcommand((sub) =>
      sub
        .setName('remove')
        ...
        .addStringOption((opt) => opt.setName('user2id').setDescription('Second user ID').setRequired(false)))
    .addSubcommand((sub) =>
      sub.setName('list').setDescription('List active restraining orders')),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'list') {
      ...
      if (!list.length) return interaction.reply({ content: 'No active restraining orders found.', ephemeral: true });
      const lines = list.map((entry: any) => `• ${formatPair(entry)}`);
      return interaction.reply({ content: lines.join('\n').slice(0, 1900), allowedMentions: { parse: [] }, ephemeral: true });
    }
    ...
    if (!user1Id || !user2Id) return interaction.reply({ content: 'Provide both users (user options or user IDs).', ephemeral: true });
    if (user1Id === user2Id) return interaction.reply({ content: 'Choose two different users.', ephemeral: true });
    ...
    if (sub === 'add') {
      ...
      return interaction.reply({ content: `Restraining order ${label} between <@${user1Id}> and <@${user2Id}>.`, allowedMentions: { parse: [] }, ephemeral: true });
    }

    if (sub === 'remove') {
      const removed = await store.remove(interaction.guild.id, user1Id, user2Id);
      if (!removed) return interaction.reply({ content: 'No restraining order found for those users.', ephemeral: true });
      return interaction.reply({ content: `Removed restraining order between <@${user1Id}> and <@${user2Id}>.`, allowedMentions: { parse: [] }, ephemeral: true });
    }
