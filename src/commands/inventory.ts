...
const tokenStore = requireFromSrcIfNeeded('../utils/messageTokenStore');
...
const { getRupeeCost } = requireFromSrcIfNeeded('../utils/economyConfig');
    ...
    .addFields(
      ...
      },
      {
        ...
        value: `**Owned:** ${rupeeBalance}\n**Cost:** ${formatCoins(
          rupeeCost
        )} coins each\n${currencyPlural} unlock the powerful /analysis command, can be granted by admins using /donate, and are spent in the configured store panel channel.`,
      }
    );
