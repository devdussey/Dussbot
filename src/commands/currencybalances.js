...
function buildUserBalanceEmbed({ guildId, user, entries }) {
  const balance = rupeeStore.getBalance(guildId, user?.id);
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'personal') {
      const entries = rupeeStore.listUserBalances(interaction.guildId, { minTokens: 0 });
