...
const logger = require('../src/utils/logger')('CleanCommands');
...
async function run() {
  ...
  try {
    if (scope === 'global' || scope === 'both') {
      if (isDryRun) {
        logger.info('[DRY-RUN] Would clear GLOBAL application commands.');
      } else {
    ...
    if (scope === 'guild' || scope === 'both') {
      if (!guildId) {
        logger.error('GUILD_ID is required to clear guild commands.');
        process.exit(1);
      }
      if (isDryRun) {
        logger.info(`[DRY-RUN] Would clear GUILD (${guildId}) application commands.`);
      } else {
        logger.info(`Clearing GUILD (${guildId}) application commands...`);
        await rest.put(Routes.applicationGuildCommands(clientId, guildId), { body: [] });
        logger.success('Guild commands cleared.');
      }
    }

    logger.info(isDryRun ? 'Dry-run complete.' : 'Done.');
  } catch (err) {
    logger.error('Failed to clear commands:', err);
    process.exit(1);
  }
}
